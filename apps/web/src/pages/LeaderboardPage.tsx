...
export function LeaderboardPage() {
  ...
  if (leaderboard.state === "not_generated") {
    return (
  ...
  if (leaderboard.state === "no_observations") {
    return (
      <section className="state-panel">
        <div className="state-symbol" aria-hidden="true">◇</div>
        <h2>暂无排行观测</h2>
        <p>当前发布版本中没有可参与排名的学生。</p>
      </section>
    );
  }

  const currentStudentNumber =
    account?.role === "student" ? account.studentNumber : null;
    ...
    <section className="panel-card leaderboard-panel">
      <header className="section-heading">
        <div>
          ...
          <h2>能力排行</h2>
          <p>
            {leaderboard.population} 名学生
            <span aria-hidden="true"> · </span>
            发布版本 {leaderboard.headRevision}
          </p>
        </div>
        ...
        <table className="leaderboard-table">
          ...
          <tbody>
            {leaderboard.items.map((item) => (
              <tr
                className={item.studentNumber === currentStudentNumber ? "current-row" : ""}
                key={item.studentNumber}
              >
                <td><span className="rank-chip">{item.rank}</span></td>
                <td className="student-cell">
                  <strong>{item.displayName ?? "未设置姓名"}</strong>
                  <span>{item.studentNumber}</span>
                </td>
                <td className="rating-cell">{item.rating}</td>
                <td>{formatMetric(item.metrics.knowledge)}</td>
                <td>{formatMetric(item.metrics.accuracy)}</td>
                <td>{formatMetric(item.metrics.quality)}</td>
                <td>{formatMetric(item.metrics.flexibility)}</td>
                <td>{formatMetric(item.metrics.proficiency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
