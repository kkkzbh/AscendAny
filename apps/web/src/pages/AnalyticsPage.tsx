...
export function AnalyticsPage() {
  ...
  if (error !== null) {
    return <ErrorPanel title="能力画像读取失败" message={error} onRetry={load} />;
  }
  if (analytics === null) return null;
  if (analytics.state === "not_generated") {
    return (
      <div className="page-stack">
        <StatePanel title="分析尚未发布" detail="管理员发布首轮分析后，这里会显示能力画像。" />
        <AchievementPanel />
        <RecommendationPanel />
      </div>
    );
  ...
  if (analytics.state === "no_observations") {
    return (
      <div className="page-stack">
        <StatePanel title="暂无个人观测" detail={"已发布分析版本 " + analytics.headRevision + "，当前账号尚无可计算的考试记录。"} />
        <AchievementPanel />
        <RecommendationPanel />
      </div>
    );
        ...
        return (
          <article className="metric-card" key={key}>
            ...
            <div className="metric-track" aria-hidden="true">
              <span style={{ width: value === null ? "0%" : String(Math.min(100, Math.max(0, value))) + "%" }} />
            </div>
          </article>
        );
      })}
    ...
    <div className="rating-chart" aria-label="Rating 历史趋势">
      {points.length === 0 ? (
        ...
        <svg viewBox="0 0 100 96" role="img" aria-label="Rating 折线图">
          ...
          <polyline points={points} fill="none" stroke="url(#rating-line)" strokeWidth="3" vectorEffect="non-scaling-stroke" />
        </svg>
      )}
    </div>
  );
