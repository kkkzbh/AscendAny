...
export async function revokeSession(
  ...
  if (current) await session.forgetLocalSession();
}
...
export async function loadAgentNotes(
  ...
  limit = 50,
  cursor?: string,
): Promise<AgentNotePage> {
  await session.ensureAuthenticated();
  const result = await listAgentNotes({
    client: session.client,
    query: { limit, ...(cursor === undefined ? {} : { cursor }) },
    throwOnError: true,
  });
...
export async function loadAgentNote(
  session: BrowserSession,
  noteId: string,
): Promise<AgentNote> {
  await session.ensureAuthenticated();
  const result = await getAgentNote({
    client: session.client,
    path: { noteId },
    throwOnError: true,
  });
...
export async function createStudentAgentNote(
  session: BrowserSession,
  title: string,
  content: string,
): Promise<AgentNoteMutationResult> {
  await session.ensureAuthenticated();
...
export async function replaceStudentAgentNote(
  session: BrowserSession,
  note: AgentNote,
  title: string,
  content: string,
  ...
  await session.ensureAuthenticated();
  const result = await replaceAgentNote({
    client: session.client,
    path: { noteId: note.id },
    body: {
      mutationId: crypto.randomUUID(),
      expectedHeadRevision: note.headRevision,
      title,
      content,
    },
    throwOnError: true,
...
export async function changeStudentAgentNoteState(
  session: BrowserSession,
  note: AgentNote,
  ...
  const result = state === "archived"
    ? await archiveAgentNote(options)
    : await restoreAgentNote(options);
  return result.data;
}

export async function sendAuthenticatedFeedback(
  session: BrowserSession,
  input: {
    title: string;
    content: string;
