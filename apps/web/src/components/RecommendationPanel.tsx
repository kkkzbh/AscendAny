...
function ModelProvenance({ model }: { model: RecommendationModelProvenance }) {
  return (
    <dl className="recommendation-provenance">
      <div><dt>模型 ID</dt><dd><code>{model.modelId}</code></dd></div>
      <div><dt>Deployment purpose</dt><dd>{model.purpose}</dd></div>
      <div><dt>Artifact</dt><dd><code>{model.artifactSha256}</code> · {model.artifactSizeBytes} bytes · mode {model.artifactMode}</dd></div>
      <div><dt>Runtime contract</dt><dd>{model.modelSchema} · {model.algorithm} · {model.inferenceContract}</dd></div>
      <div><dt>模型 head</dt><dd>r{model.modelHeadRevision} · trained {model.trainedAt}</dd></div>
      <div><dt>Training provenance</dt><dd><code>{model.trainingProvenanceSha256}</code></dd></div>
      <div><dt>Feature schema</dt><dd><code>{model.featureSchemaSha256}</code></dd></div>
      <div><dt>知识目录</dt><dd><code>{model.knowledgeCatalogSha256}</code></dd></div>
      <div><dt>Parameters</dt><dd><code>{model.parameterSha256}</code></dd></div>
      <div><dt>Golden vectors</dt><dd><code>{model.goldenVectorsSha256}</code></dd></div>
      <div><dt>应用 release</dt><dd>{model.applicationVersion} · <code>{model.applicationCommit}</code> · {model.applicationBuildTime}</dd></div>
    </dl>
  );
}
...
export function RecommendationPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setRecommendation(await loadSelfRecommendation(session));
    } catch (loadError) {
  ...
  if (error !== null) {
    return <section className="panel-card recommendation-panel error-state"><h2>学习建议读取失败</h2><p>{error}</p><button className="secondary-button" type="button" onClick={() => void load()}>重试</button></section>;
  }
  ...
  if (recommendation.state === "unavailable") {
    return (
      <section className="panel-card recommendation-panel">
        <header className="section-heading"><div><span className="eyebrow">INFERENCE</span><h2>学习建议暂不可用</h2><p>{unavailableMessage(recommendation.unavailableReason)}</p></div></header>
        <p className="recommendation-provenance">分析 head r{recommendation.currentAnalyticsHeadRevision} · 模型 head r{recommendation.modelHeadRevision}{recommendation.currentAnalyticsGenerationId === undefined ? "" : ` · generation ${recommendation.currentAnalyticsGenerationId}`}</p>
  ...
  return (
    <section className="panel-card recommendation-panel">
      ...
      <div className="recommendation-grid">
        {recommendation.result.knowledgeMastery.map((knowledge) => <article key={knowledge.knowledgePointId}><span>知识点掌握度</span><strong>{knowledge.label} · {percentage(knowledge.mastery)}</strong><p>{knowledge.description}</p><p>观测 {knowledge.observationCount} 条</p></article>)}
      </div>
