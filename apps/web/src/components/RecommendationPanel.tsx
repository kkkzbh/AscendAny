...
export function RecommendationPanel() {
  ...
  if (loading) {
    return <section className="panel-card recommendation-panel" role="status"><p>正在读取学习建议…</p></section>;
  }
  if (error !== null) {
    return (
      <section className="panel-card recommendation-panel error-state">
        <h2>学习建议读取失败</h2><p>{error}</p>
  ...
  if (recommendation.state === "unavailable") {
    const reason = recommendation.unavailableReason === "no_active_model"
      ? "管理员尚未发布推荐模型。"
      : "当前推荐模型尚未覆盖你的分析记录。";
    return (
      <section className="panel-card recommendation-panel">
        <header className="section-heading"><div><span className="eyebrow">LEARNING PATH</span><h2>学习建议待生成</h2><p>{reason}</p></div></header>
        <p className="recommendation-provenance">分析 head r{recommendation.currentAnalyticsHeadRevision} · 推荐 head r{recommendation.recommendationHeadRevision}</p>
      </section>
    );
  }
    ...
    <section className="panel-card recommendation-panel">
      <header className="section-heading">
        <div>
          ...
          <h2>个性化学习建议</h2>
          <p>基于 Rating {recommendation.result.sourceRating} 与已发布模型生成。</p>
        </div>
        <span className={`recommendation-state ${recommendation.state}`}>{recommendation.state === "fresh" ? "最新" : "待更新"}</span>
      </header>
      ...
      <div className="recommendation-grid">
        {recommendation.result.knowledgeMastery.map((knowledge) => <article key={knowledge.knowledgePointId}><span>知识点掌握度</span><strong>{knowledge.label} · {percentage(knowledge.mastery)}</strong><p>{knowledge.description}</p><p>训练证据 {knowledge.trainInteractionCount} 条</p></article>)}
      </div>
      ...
      <dl className="recommendation-provenance">
        <div><dt>模型</dt><dd>{recommendation.model.modelSchema}</dd></div>
        <div><dt>训练配置</dt><dd>{recommendation.model.trainingConfigurationKey} v{recommendation.model.trainingConfigurationVersion}</dd></div>
        <div><dt>知识目录</dt><dd>{recommendation.model.knowledgeCatalogKey} v{recommendation.model.knowledgeCatalogVersion}</dd></div>
        <div><dt>分析来源</dt><dd>generation {recommendation.model.analyticsGenerationId} · head r{recommendation.model.analyticsHeadRevision}</dd></div>
      </dl>
    </section>
  );
}
