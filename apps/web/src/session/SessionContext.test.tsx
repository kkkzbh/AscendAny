...
describe("SessionProvider", () => {
  it("keeps a bootstrap failure visible until the user clears it", async () => {
  ...
  it("bootstraps and delegates login and logout to BrowserSession", async () => {
    const session = fakeBrowserSession();
    const { result } = renderHook(() => useSession(), {
      wrapper: wrapperFor(session),
    });
  ...
  it("delegates an explicitly supplied enrollment claim", async () => {
    const session = fakeBrowserSession();
    const { result } = renderHook(() => useSession(), {
      wrapper: wrapperFor(session),
    });
