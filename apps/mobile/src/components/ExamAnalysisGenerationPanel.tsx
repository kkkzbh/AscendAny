  ...
  return (
    <section className="panel-card exam-generation-panel">
      <header className="section-heading">
        <div><span className="eyebrow">ANALYSIS GENERATION</span><h2>考试分析生成</h2><p>generation #{generation.generationId} · event head {generation.eventHead}</p></div>
      ...
      <dl className="exam-generation-summary">
        <div><dt>尝试</dt><dd>{generation.attemptCount}</dd></div>
        <div><dt>创建</dt><dd>{formatTimestamp(generation.createdAt)}</dd></div>
        <div><dt>开始</dt><dd>{generation.startedAt === undefined ? "—" : formatTimestamp(generation.startedAt)}</dd></div>
        <div><dt>完成</dt><dd>{generation.finishedAt === undefined ? "—" : formatTimestamp(generation.finishedAt)}</dd></div>
      </dl>
      <p className="exam-generation-connection" data-state={state.connectionState ?? "idle"}>{connectionLabel(state.connectionState, lastSequence, state.error !== null)}</p>
      ...
      {state.events.length === 0 ? <p className="empty-copy">本次打开后尚未收到新事件。</p> : (
        <ol className="exam-generation-events" aria-label="分析生成事件">
          {state.events.map((event) => <li key={event.sequence}><span>#{event.sequence}</span><strong>{statusLabel[event.type]}</strong><time dateTime={event.createdAt}>{formatTimestamp(event.createdAt)}</time></li>)}
        </ol>
      )}
    </section>
  );
