...
describe("Mobile ChatView", () => {
  ...
  it("renders the durable assistant reply after its SSE stream completes", async () => {
    render(<ChatView />);
    ...
    expect(operations.loadAgentRun).toHaveBeenCalledWith(session, run.id);
  });

  it("queues ready analytics and follows the dedicated pinned run", async () => {
    ...
    expect(await screen.findByText(automaticOutput.content)).not.toBeNull();
  ...
  it("restores and displays an existing automatic-analysis run", async () => {
    ...
    expect(await screen.findByText(automaticOutput.content)).not.toBeNull();
    expect(screen.getByText(automaticInput.content)).not.toBeNull();
    expect(operations.openAgentRunEventStream).toHaveBeenCalledWith(
      session,
      automaticRun.id,
      0,
  ...
  it("uses an advanced analytics head for the next automatic run", async () => {
    ...
    const outputAtHead8: ChatMessage = {
      ...
      sequence: 4,
      content: "移动端自动分析：head 8 已纳入最新考试。",
      runId: runAtHead8.id,
    };
    ...
    expect(await screen.findByText(outputAtHead8.content)).not.toBeNull();
    expect(operations.enqueueAutomaticAnalysis).toHaveBeenNthCalledWith(2, session, {
      promptConfigurationKey: "agent.prompt.default",
      modelConfigurationKey: "agent.model.default",
  ...
  ])("does not enqueue for a non-ready analytics state", async (analytics, message) => {
  ...
  it("shows retry for a head conflict and missing configuration", async () => {
    operations.loadSelfAnalytics.mockResolvedValue(readyAnalytics(7));
  ...
  it("resumes a disconnected automatic stream from the durable cursor", async () => {
    configureAutomaticAnalysis(false);
    let streamCount = 0;
    operations.openAgentRunEventStream.mockImplementation(async () => {
      streamCount += 1;
