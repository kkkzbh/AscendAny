...
export function ChatView() {
  ...
  return (
    <div className="desktop-chat">
      <aside className="desktop-chat-threads">
        <button className="primary-button compact" type="button" disabled={creating} onClick={() => void createThread()}>
          {creating ? "正在创建…" : "新建对话"}
        </button>
        <div className="desktop-chat-thread-list" aria-label="对话列表">
          {loadingThreads && threads.length === 0 ? <p>正在读取对话…</p> : null}
          {!loadingThreads && threads.length === 0 ? <p>还没有对话。</p> : null}
        ...
        {nextThreadCursor !== null ? <button className="text-button" type="button" onClick={() => void loadMoreThreads()}>加载更多</button> : null}
      </aside>

      <section className="desktop-chat-conversation">
        <div className={`auto-analysis-status ${automaticAnalysisState.phase}`} role="status">
          <div><strong>自动学习分析</strong><span>{automaticAnalysisState.message}</span></div>
          <button
        ...
        {selectedThread === null ? (
          <div className="desktop-chat-empty"><span>◇</span><h2>创建一个对话</h2><p>Agent 回复、工具调用和事件都会持久化。</p></div>
        ) : (
          <>
            <header className="desktop-chat-header">
              <div><strong>{selectedThread.kind === "auto_analysis" ? "自动学习分析" : "学习助手"}</strong><span>更新于 {formatTime(selectedThread.updatedAt)}</span></div>
              {running ? <span className="desktop-run-status">{eventType ?? activeRun.status}</span> : null}
            </header>
            <div className="desktop-chat-messages" aria-live="polite">
              ...
              {loadingMessages && messages.length === 0 ? <p>正在读取消息…</p> : null}
              {!loadingMessages && messages.length === 0 ? (
                <p>{selectedThread.kind === "auto_analysis" ? "正在等待自动分析结果。" : "发送第一条消息开始分析。"}</p>
              ) : null}
              {messages.map((message) => <Message key={message.id} message={message} />)}
              {running ? <article className="desktop-chat-message assistant pending"><span>Agent 正在处理</span><strong>{eventType ?? activeRun.status}</strong></article> : null}
            </div>
            {running && error !== null ? (
              <button className="text-button desktop-chat-resume" type="button" onClick={() => void watchRun(activeRun, eventSequence, inputSequence)}>
...
function Message({ message }: { message: ChatMessage }) {
  ...
  return (
    <article className={`desktop-chat-message ${assistant ? "assistant" : "user"}`}>
      <header><strong>{author}</strong><span>#{message.sequence} · {formatTime(message.createdAt)}</span></header>
      <p>{message.content}</p>
