...
func NewDefaultRateLimiter() (*FixedWindowRateLimiter, error) {
	return NewFixedWindowRateLimiter(map[string]RateLimit{
		...
		"agent.chat.reply":                  {Requests: 60, Window: time.Minute},
		"agent.chat.auto-analysis":          {Requests: 30, Window: time.Minute},
		"agent.auth.policy":                 {Requests: 120, Window: time.Minute},
		"agent.auth.register":               {Requests: 10, Window: 15 * time.Minute},
		"auth.register.username":            {Requests: 5, Window: 15 * time.Minute},
		"agent.auth.login":                  {Requests: 20, Window: 5 * time.Minute},
		"agent.auth.sso.exchange":           {Requests: 10, Window: 15 * time.Minute},
		"agent.auth.refresh":                {Requests: 60, Window: time.Minute},
		"agent.auth.logout":                 {Requests: 30, Window: time.Minute},
		"agent.auth.me":                     {Requests: 120, Window: time.Minute},
		"agent.auth.profile":                {Requests: 30, Window: time.Minute},
		"agent.auth.local-password.bootstrap": {
			Requests: 10,
			Window:   15 * time.Minute,
		},
		"agent.students.dashboard":          {Requests: 120, Window: time.Minute},
		"agent.students.achievements":       {Requests: 120, Window: time.Minute},
		"agent.students.leaderboard":        {Requests: 120, Window: time.Minute},
		"agent.meta.latest-exam":            {Requests: 120, Window: time.Minute},
		"agent.meta.data-events":            {Requests: 30, Window: time.Minute},
		"agent.recommendations.path":        {Requests: 120, Window: time.Minute},
		"agent.recommendations.path-status": {Requests: 120, Window: time.Minute},
		"agent.recommendations.knowledge":   {Requests: 120, Window: time.Minute},
		"agent.feedback.submit":             {Requests: 30, Window: time.Minute},
