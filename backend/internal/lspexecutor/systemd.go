...
func (launcher *SystemdLauncher) Start(ctx context.Context, sessionID string) error {
	return launcher.control(ctx, "start", sessionID)
}

func (launcher *SystemdLauncher) Stop(ctx context.Context, sessionID string) error {
	return launcher.control(ctx, "stop", sessionID)
}

func (launcher *SystemdLauncher) control(ctx context.Context, verb, sessionID string) error {
	if ctx == nil || !lsp.ValidPublicID(sessionID) || (verb != "start" && verb != "stop") {
		return errors.New("canonical LSP session ID and systemd verb are required")
	}
	unit := "ascendany-lsp@" + sessionID + ".service"
	...
	if err != nil {
		...
		return fmt.Errorf("systemctl %s LSP unit: %w: %s", verb, err, strings.ToValidUTF8(string(output), "�"))
	}
	return nil
}
