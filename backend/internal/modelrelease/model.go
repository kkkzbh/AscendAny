...
const advisoryLockID int64 = 0x4153434d4f44454c
...
func (repository *Repository) Bind(
	...
	release, err := prepareRelease(loaded, application)
	if err != nil {
		return Binding{}, err
	}
	...
	return Binding{
		...
		ModelPurpose: release.modelPurpose,
...
WHERE head.singleton`).Scan(
	...
	finished = true
	return Binding{
		ReleaseID: releaseID, HeadRevision: headRevision, Activated: false,
		ModelPurpose: release.modelPurpose,
		ManifestJSON: append(json.RawMessage(nil), release.manifestJSON...), ManifestSHA256: release.manifestSHA256,
	}, nil
}
...
func prepareRelease(loaded modelartifact.Loaded, application ApplicationIdentity) (preparedRelease, error) {
...
func validateApplicationIdentity(identity ApplicationIdentity) error {
	for name, value := range map[string]string{
		"version": identity.Version, "commit": identity.Commit, "build time": identity.BuildTime,
	} {
		if value == "" || strings.TrimSpace(value) != value || len(value) > 128 {
			return fmt.Errorf("%w: application %s must be nonempty, trimmed, and at most 128 bytes", ErrInvalidConfiguration, name)
		}
	}
	return nil
}
...
func ensureHead(
	...
	err := tx.QueryRow(ctx, `
SELECT current_release_id, head_revision
FROM ascendany.recommendation_model_head
WHERE singleton
FOR UPDATE`).Scan(&currentReleaseID, &currentRevision)
	if errors.Is(err, pgx.ErrNoRows) {
		if err := insertActivation(ctx, tx, 1, releaseID, release.artifactSHA256, application); err != nil {
			return 0, false, err
		}
...
WHERE head_revision = $1
  AND recommendation_model_release_id = $2`, currentRevision, releaseID).Scan(
			&artifactSHA256, &version, &commit, &buildTime,
		); err != nil {
			return 0, false, fmt.Errorf("read current model activation event: %w", err)
		}
		if artifactSHA256 != release.artifactSHA256 {
			return 0, false, fmt.Errorf("%w: current activation artifact differs from the model release", ErrStoredDataInvalid)
		}
		if version == application.Version && commit == application.Commit && buildTime == application.BuildTime {
			return currentRevision, false, nil
		}
	}
	nextRevision := currentRevision + 1
	if err := insertActivation(ctx, tx, nextRevision, releaseID, release.artifactSHA256, application); err != nil {
		return 0, false, err
	}
...
WHERE singleton
  ...
  AND head_revision = $4`, releaseID, nextRevision, currentReleaseID, currentRevision)
	if err != nil {
...
INSERT INTO ascendany.recommendation_model_activation_events (
    ...
    application_version, application_commit, application_build_time
		...
		revision, releaseID, artifactSHA256, application.Version, application.Commit, application.BuildTime,
	); err != nil {
