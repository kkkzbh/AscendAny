...
| Agent frontend（Electron 与 `/app/`） | 现有 `apps/desktop/` TypeScript source | 用户设备或 Go embedded static delivery |
| Mobile、Import Console、Site | TypeScript strict mode | 用户设备或 Go embedded static delivery |
...
外部模块交付一个已经训练完成的 canonical `ascendany.recommendation.inference-model.v1` artifact 及独立记录的 SHA-256。Artifact、release manifest、runtime config 与 schema-v10 model/catalog provenance 都携带相同的 deployment `purpose`。生产构建、安装、启动和最终 validator 固定要求 `production`；full-E2E 使用显式 `acceptance_test` capability，且 acceptance fixture 不能进入 production path。Release builder 与启动路径使用 Go 验证 closed schema、fixed algorithm、feature schema、knowledge catalog digest、parameter digest、golden-vector digest、numeric envelope 和全部 golden results。`ascendanyd` 将 artifact identity、release version、Git commit 与 build time 绑定到 immutable schema-v10 model release/head 与 catalog publication graph，并在请求读取事务中执行 deterministic Go inference。

## 2. Fresh data boundary

- v2 使用独立 PostgreSQL database `ascendany_v2`；schema-v10 fresh inventory 固定为 54 张 base tables、31 条 sequences 与 10 条连续 migration history。
...
`apps/desktop/` 是用户原 Agent 前端的冻结 product source，同一套 source 生成 Electron 客户端和 production `/app/` Web build。Go backend 按该前端已有的 `/api/v1` JSON/SSE shape 提供认证、学生数据、Agent stream、数据刷新 stream 与 recommendation projection。界面、交互、本地 notes/chat/settings state 和 client-side normalization 保持原实现。`apps/web/` 的替换版 Web 界面不进入 product delivery。
...
Agent frontend 的 `/api/v1` endpoint shape 保持已有 client contract，Go 直接实现该 product boundary。新增 `/api/v2` TypeScript clients 由 `contracts/openapi/ascendany-v2.yaml` 生成。架构升级集中在 Go service、transaction、durable job、model inference 与 deploy ownership，不重写 Agent 前端。
...
- 替换原 Agent 前端的旧 Web 产品路径及其 production public assets；
